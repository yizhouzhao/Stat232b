...
/*Graph Functions*/
                                ...
                                const std::shared_ptr<TE> attr,bool multi_edge, int pos)
    {
        ...
        if(!multi_edge &&
           std::find_if(this->adjacency_lists_[source].begin(), this->adjacency_lists_[source].end(),
                        [target](const std::shared_ptr<Edge> edge) {return edge->target() == target;})
    ...
    bool Graph<TV, TE>::DeleteEdge(VertexId source, VertexId target)
    {
            ...
            !(this->vertices_[source] && this->vertices_[target]))
        {
            ...
            return false;
                 ...
                 == this->adjacency_lists_[source].end())
        {
            std::cerr << "Edge from " << source << " to " << target << 
                " doesn't exist (not an error in AddRule() / ReconstructTree())" << std::endl;
            return false;
        }
        else
        {
